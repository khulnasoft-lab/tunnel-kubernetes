...
import (
	...
	"github.com/khulnasoft-lab/tunnel-kubernetes/pkg/jobs"
...
func (c *client) Namespace(namespace string) TunnelK8S {
	c.namespace = namespace
	return c
}

// Namespace configure the namespace to execute the queries
func (c *client) AllNamespaces() TunnelK8S {
	c.allNamespaces = true
	return c
}

// Resource configure which resources to execute the queries
func (c *client) Resources(resources string) TunnelK8S {
	if len(resources) == 0 {
		return c
	}
...
func (c *client) ListArtifactAndNodeInfo(ctx context.Context, namespace string, ignoreLabels map[string]string, tolerations ...corev1.Toleration) ([]*artifacts.Artifact, error) {
	...
	labels := map[string]string{
		jobs.TunnelCollectorName: jobs.NodeCollectorName,
		jobs.TunnelAutoCreated:   "true",
	}
	...
	for _, resource := range artifactList {
		...
		nodeLabels := map[string]string{
			jobs.TunnelResourceName: resource.Name,
			jobs.TunnelResourceKind: resource.Kind,
		}
		// append node labels
