...
func (jb *jobCollector) ApplyAndCollect(ctx context.Context, nodeName string) (string, error) {
	...
	_, err = jb.getTunnelNamespace(ctx)
	if err != nil {
		if k8sapierror.IsNotFound(err) {
			tunnelNamespace := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: jb.namespace}}
			_, err = jb.cluster.GetK8sClientSet().CoreV1().Namespaces().Create(ctx, tunnelNamespace, metav1.CreateOptions{})
			if err != nil {
				return "", err
			}
		}
...
func (jb *jobCollector) deleteTunnelNamespace(ctx context.Context) {
	background := metav1.DeletePropagationBackground
...
func (jb *jobCollector) getTunnelNamespace(ctx context.Context) (*corev1.Namespace, error) {
